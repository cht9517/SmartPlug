...
namespace SmartPlug
{
    public class Tsb
    {
        private static Tsb instance = new Tsb();
        private Tsb() { }
        public static Tsb Instance { get { return instance; } }

        public delegate void tsb_callback(int com_id);
        ...
        public static byte[,] rx_msg_buf = new byte[257,1050];

        private byte rx_state = 0;
        private bool rx_escape = false;
        
        private static int rx_len = 0;
        private int rx_msg_len = 0;
        private static int rx_cmd_id = 0;

        private bool tsb_rx_byte(byte dat)
        {
            if (rx_state == 0)
            {
                if (dat == (byte)(0xFB))
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        rx_state = 0;
                        rx_msg_len = ((rx_buf_o[1] & 0xff) << 8) + (rx_buf_o[2] & 0xff);
                        if (rx_msg_len + 5 == rx_len)//包中长度信息正确
                        {
                            int crc = CRC16(rx_buf_o, 3, rx_msg_len);
        ...
        public bool tsb_tx_frame(int tool_id, int cmd_id, byte[] buf,  tsb_callback cb)
        {
            ...
            tx_buf_o[1] = (byte)((len >> 8) & 0xff);
            tx_buf_o[2] = (byte)(len & 0xff);

            if (++ComID >= 256)
                ComID = 0;
            ComID = 0;
            //tx_buf_o[3] = (byte)((ComID >> 8) & 0xff);
            tx_buf_o[3] = 1;
            tx_buf_o[4] = (byte)(ComID & 0xff);
            tx_buf_o[5] = (byte)(tool_id & 0xff);
            tx_buf_o[6] = (byte)(cmd_id  & 0xff);
            tx_buf_o[7] = 0;

            //拷贝参数
            if (buf != null)
                Buffer.BlockCopy(buf, 0, tx_buf_o, 8, buf.Length);

            //计算CRC
            int tx_crc = CRC16(tx_buf_o, 3, len);
            tx_buf_o[len + 3] = (byte)((tx_crc >> 8) & 0xff);
            tx_buf_o[len + 4] = (byte)(tx_crc & 0xff);
            ...
            {
                //var task_tsb_tx_rx = Task.Factory.StartNew(() =>
                //{
        ...
        private static void tsb_rx_thd()
        {
            while (true)
            {
                Thread.Sleep(20);
                ...
                if (sPort.IsOpen)
                {
                    ...
                    while (id < rx_cnt)
                    {
                        if (Tsb.instance.tsb_rx_byte(rx_buf[id++]))//成功接收到一帧
                        {
                            //位置映射
                            if (rx_cmd_id >= 0x100)
                                rx_cmd_id = rx_cmd_id & 0xff;
                            else
                                rx_cmd_id = 0x100;

                            for (int i = 0; i < rx_len; i++)
                                rx_msg_buf[rx_cmd_id, i] = rx_buf_o[i];
                            
                            if (tsb_cb_buf[rx_cmd_id] != null)
                                tsb_cb_buf[rx_cmd_id](rx_cmd_id);
                            /*
        ...
        private int CRC16(byte[] buf, int offset, int len)
        {
            int crc, i;
